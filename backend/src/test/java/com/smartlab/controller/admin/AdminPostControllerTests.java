...
class AdminPostControllerTests {
	...
	@Test
	void deletePostReturnsNoContentAndPassesActorAndPostId() throws Exception {
		UUID actorUserId = UUID.randomUUID();
		UUID postId = UUID.randomUUID();
		when(actorResolver.requireActorUserId()).thenReturn(actorUserId);
		...
		ArgumentCaptor<AdminPostService.DeleteAdminPostCommand> captor =
				ArgumentCaptor.forClass(AdminPostService.DeleteAdminPostCommand.class);
		verify(adminPostService).softDeletePost(captor.capture());
		verifyNoMoreInteractions(adminPostService);
		assertEquals(actorUserId, captor.getValue().actorUserId());
		assertEquals(postId, captor.getValue().postId());
	}

	@Test
	void deletePostRequestWithoutAuthenticatedActorIsUnauthorized() throws Exception {
		when(actorResolver.requireActorUserId())
				.thenThrow(new AuthenticationCredentialsNotFoundException("Authentication is required."));

		mockMvc.perform(delete("/api/admin/posts/{postId}", UUID.randomUUID()))
				.andExpect(status().isUnauthorized())
				.andExpect(jsonPath("$.status").value(401));
	}

	@Test
	void malformedDeletePostIdReturnsBadRequest() throws Exception {
		when(actorResolver.requireActorUserId()).thenReturn(UUID.randomUUID());

		mockMvc.perform(delete("/api/admin/posts/not-a-uuid"))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.message").value("Request validation failed."));
	}

	@Test
	void deletePostMissingResourceUsesGenericNotFoundMessage() throws Exception {
		when(actorResolver.requireActorUserId()).thenReturn(UUID.randomUUID());
		doThrow(new ResourceNotFoundException("Post was not found."))
				.when(adminPostService)
				.softDeletePost(any(AdminPostService.DeleteAdminPostCommand.class));
